...
Then, we initialize the following `dicts`:
...
PAYPAL_HEADERS = {
    ...
    'X-PAYPAL-APPLICATION-ID' : 'APP-80W284485P519543T',  # this is sandbox value, same for everyone
...
PAYLOAD = {
    ...
    'feesPayer': 'EACHRECEIVER', # optional. Allowed values: SENDER, PRIMARYRECEIVER, EACHRECEIVER, SECONDARYONLY
...
r = requests.post(PAYPAL_API_ENDPOINT_PAY, data=json.dumps(payload_check_verify), headers=PAYPAL_HEADERS)
